...
typedef struct {
    ...
    float max_out;       // 最大输出速度
    float max_iout;      // 积分最大值（防止过大）
    float deadband;      // 死区（|err| < deadband 就输出 0）
    float d_filter;      // 微分滤波系数，范围0~1，越小越平滑
...
//    float ControlPeriod;
    ...
    float Derivative_Filtering_Coefficient;//0-1的分数,越小受上一次的微分结果影响越大
